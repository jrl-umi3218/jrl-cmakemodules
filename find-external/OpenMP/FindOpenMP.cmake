...
function(_OPENMP_FLAG_CANDIDATES LANG)
  if(NOT OpenMP_${LANG}_FLAG)
    ...
    elseif(
      ...
      AND "${CMAKE_${LANG}_COMPILER_VERSION}" VERSION_LESS "15.0.0.20140528"
    )
    ...
    if(
      CMAKE_${LANG}_COMPILER_ID STREQUAL "IntelLLVM"
...
function(_OPENMP_GET_FLAGS LANG FLAG_MODE OPENMP_FLAG_VAR OPENMP_LIB_NAMES_VAR)
  ...
  foreach(OPENMP_FLAG IN LISTS OpenMP_${LANG}_FLAG_CANDIDATES)
    ...
    if(OpenMP_COMPILE_RESULT_${FLAG_MODE}_${OPENMP_PLAIN_FLAG})
      ...
      break()
    elseif(
      CMAKE_${LANG}_COMPILER_ID STREQUAL "AppleClang"
...
function(_OPENMP_GET_SPEC_DATE LANG SPEC_DATE)
  ...
  string(REGEX REPLACE "[-/=+]" "" OPENMP_PLAIN_FLAG "${OPENMP_FLAG}")
  try_compile(
    OpenMP_SPECTEST_${LANG}_${OPENMP_PLAIN_FLAG}
    SOURCE_FROM_VAR "${_OPENMP_TEST_SRC_NAME}" _OPENMP_TEST_SRC_CONTENT
    LOG_DESCRIPTION "Detecting ${LANG} OpenMP version"
    CMAKE_FLAGS
      "-DCOMPILE_DEFINITIONS:STRING=${OpenMP_${LANG}_FLAGS}"
      ${_includeDirFlags}
    COPY_FILE "${BIN_FILE}"
  )
    ...
    if(
      ...
      if(
        ...
        set(
          ...
          "${OpenMP_${LANG}_FLAGS_WORK}"
          CACHE STRING
          "${LANG} compiler flags for OpenMP parallelization"
          FORCE
        )
      endif()
      if(
        NOT DEFINED OpenMP_${LANG}_LIB_NAMES
        OR "${OpenMP_${LANG}_LIB_NAMES}" STREQUAL "NOTFOUND"
      )
        set(
          OpenMP_${LANG}_LIB_NAMES
          "${OpenMP_${LANG}_LIB_NAMES_WORK}"
          CACHE STRING
          "${LANG} compiler libraries for OpenMP parallelization"
          FORCE
        )
      endif()
...
if(CMAKE_Fortran_COMPILER_LOADED)
  if(
    NOT DEFINED OpenMP_Fortran_FLAGS
    ...
    if(
      NOT DEFINED OpenMP_Fortran_LIB_NAMES
      OR "${OpenMP_Fortran_LIB_NAMES}" STREQUAL "NOTFOUND"
    )
      set(
  ...
  if(CMAKE_${LANG}_COMPILER_LOADED)
    ...
    if(OpenMP_${LANG}_FOUND)
      if(DEFINED OpenMP_${LANG}_VERSION)
        if(
          NOT _OpenMP_MIN_VERSION
          OR _OpenMP_MIN_VERSION VERSION_GREATER OpenMP_${LANG}_VERSION
        )
      ...
      if(OpenMP_${LANG}_FLAGS)
        ...
        if(
          CMAKE_${LANG}_COMPILER_ID STREQUAL "Fujitsu"
          OR ${CMAKE_${LANG}_COMPILER_ID} STREQUAL "IntelLLVM"
        )
          set_property(
            TARGET OpenMP::OpenMP_${LANG}
            PROPERTY INTERFACE_LINK_OPTIONS "${OpenMP_${LANG}_FLAGS}"
          )
        endif()
      ...
      if(OpenMP_${LANG}_LIBRARIES)
        set_property(
          TARGET OpenMP::OpenMP_${LANG}
          PROPERTY INTERFACE_LINK_LIBRARIES "${OpenMP_${LANG}_LIBRARIES}"
        )
      endif()
    endif()
  endif()
...
foreach(LANG IN ITEMS C CXX Fortran)
  if(
    (NOT OpenMP_FIND_COMPONENTS AND CMAKE_${LANG}_COMPILER_LOADED)
