...
function(_OPENMP_FLAG_CANDIDATES LANG)
  if(NOT OpenMP_${LANG}_FLAG)
    ...
    if(CMAKE_${LANG}_COMPILER_ID STREQUAL "IntelLLVM"
       AND "x${CMAKE_${LANG}_COMPILER_FRONTEND_VARIANT}" STREQUAL "xMSVC")
    ...
    set(OpenMP_${LANG}_FLAG_CANDIDATES
...
# in Fortran, an implementation may provide an omp_lib.h header or omp_lib
# module, or both (OpenMP standard, section 3.1) Furthmore !$ is the Fortran
# equivalent of #ifdef _OPENMP (OpenMP standard, 2.2.2) Without the conditional
# compilation, some compilers (e.g. PGI) might compile OpenMP code while not
# actually enabling OpenMP, building code sequentially
...
macro(_OPENMP_PREPARE_SOURCE LANG CONTENT_ID NAME_PREFIX FULLNAME_VAR
      CONTENT_VAR)
  if("${LANG}" STREQUAL "C")
...
function(_OPENMP_GET_FLAGS LANG FLAG_MODE OPENMP_FLAG_VAR OPENMP_LIB_NAMES_VAR)
  ...
  separate_arguments(OpenMP_VERBOSE_OPTIONS NATIVE_COMMAND
  ...
  foreach(OPENMP_FLAG IN LISTS OpenMP_${LANG}_FLAG_CANDIDATES)
    ...
    try_compile(
      OpenMP_COMPILE_RESULT_${FLAG_MODE}_${OPENMP_PLAIN_FLAG} SOURCE_FROM_VAR
      "${_OPENMP_TEST_SRC_NAME}" _OPENMP_TEST_SRC_CONTENT LOG_DESCRIPTION
      "Detecting ${LANG} OpenMP compiler info"
    ...
    if(OpenMP_COMPILE_RESULT_${FLAG_MODE}_${OPENMP_PLAIN_FLAG})
      ...
      if(CMAKE_${LANG}_VERBOSE_FLAG)
        ...
        # For LCC we should additionally alanyze -print-search-dirs output to
        # check for additional implicit_dirs. Note: This won't work if CMP0129
        # policy is set to OLD!
        if("${CMAKE_${LANG}_COMPILER_ID}" STREQUAL "LCC")
          execute_process(
            COMMAND ${CMAKE_${LANG}_COMPILER} -print-search-dirs
            OUTPUT_VARIABLE output_lines COMMAND_ERROR_IS_FATAL ANY
            ERROR_QUIET)
          if("${output_lines}" MATCHES ".*\nlibraries:[ \t]+(.*:)\n.*")
            string(REPLACE ":" ";" implicit_dirs_addon "${CMAKE_MATCH_1}")
            list(PREPEND OpenMP_${LANG}_IMPLICIT_LINK_DIRS
                 ${implicit_dirs_addon})
            string(
        ...
        foreach(_OPENMP_IMPLICIT_LIB IN LISTS OpenMP_${LANG}_IMPLICIT_LIBRARIES)
          get_filename_component(_OPENMP_IMPLICIT_LIB_DIR
                                 "${_OPENMP_IMPLICIT_LIB}" DIRECTORY)
          get_filename_component(_OPENMP_IMPLICIT_LIB_NAME
                                 "${_OPENMP_IMPLICIT_LIB}" NAME)
          get_filename_component(_OPENMP_IMPLICIT_LIB_PLAIN
                                 "${_OPENMP_IMPLICIT_LIB}" NAME_WE)
          string(
            REGEX
            REPLACE "([][+.*?()^$])" "\\\\\\1" _OPENMP_IMPLICIT_LIB_PLAIN_ESC
                    "${_OPENMP_IMPLICIT_LIB_PLAIN}")
          string(
            REGEX
            REPLACE "([][+.*?()^$])" "\\\\\\1" _OPENMP_IMPLICIT_LIB_PATH_ESC
                    "${_OPENMP_IMPLICIT_LIB}")
          if(NOT
             (
             "${_OPENMP_IMPLICIT_LIB}" IN_LIST
             CMAKE_${LANG}_IMPLICIT_LINK_LIBRARIES
             OR "${CMAKE_${LANG}_STANDARD_LIBRARIES}"
                MATCHES
                "(^| )(-Wl,)?(-l)?(${_OPENMP_IMPLICIT_LIB_PLAIN_ESC}|${_OPENMP_IMPLICIT_LIB_PATH_ESC})( |$)"
             OR "${CMAKE_${LANG}_LINK_EXECUTABLE}"
                MATCHES
                "(^| )(-Wl,)?(-l)?(${_OPENMP_IMPLICIT_LIB_PLAIN_ESC}|${_OPENMP_IMPLICIT_LIB_PATH_ESC})( |$)"
             ))
            if(_OPENMP_IMPLICIT_LIB_DIR)
              set(OpenMP_${_OPENMP_IMPLICIT_LIB_PLAIN}_LIBRARY
                  "${_OPENMP_IMPLICIT_LIB}"
                  CACHE
                    FILEPATH
              ...
              find_library(
                OpenMP_${_OPENMP_IMPLICIT_LIB_PLAIN}_LIBRARY
                ...
                CMAKE_FIND_ROOT_PATH_BOTH NO_PACKAGE_ROOT_PATH NO_CMAKE_PATH
        ...
        # compiler. Assume that it handles them automatically, e.g. the Intel
        # Compiler on Windows should put the dependency in its object files.
        set("${OPENMP_LIB_NAMES_VAR}"
            ""
            PARENT_SCOPE)
      endif()
    ...
    elseif(CMAKE_${LANG}_COMPILER_ID STREQUAL "AppleClang"
      ...
      if(OpenMP_libomp_LIBRARY)
        ...
        try_compile(
          OpenMP_COMPILE_RESULT_${FLAG_MODE}_${OPENMP_PLAIN_FLAG}
          SOURCE_FROM_VAR
          "${_OPENMP_TEST_SRC_NAME}" _OPENMP_TEST_SRC_CONTENT LOG_DESCRIPTION
          "Trying ${LANG} OpenMP compiler with '${OpenMP_libomp_LIBRARY}'"
          CMAKE_FLAGS "-DCOMPILE_DEFINITIONS:STRING=${OPENMP_FLAGS_TEST}"
        ...
        if(NOT OpenMP_COMPILE_RESULT_${FLAG_MODE}_${OPENMP_PLAIN_FLAG})
          ...
          set(OpenMP_${LANG}_INCLUDE_DIR
              "${OpenMP_${LANG}_INCLUDE_DIR}"
              PARENT_SCOPE)
          if(OpenMP_${LANG}_INCLUDE_DIR)
            try_compile(
              OpenMP_COMPILE_RESULT_${FLAG_MODE}_${OPENMP_PLAIN_FLAG}
              ...
              LOG_DESCRIPTION
              "Trying ${LANG} OpenMP compiler with '${OpenMP_libomp_LIBRARY}' and '${OpenMP_${LANG}_INCLUDE_DIR}'"
              CMAKE_FLAGS
                "-DCOMPILE_DEFINITIONS:STRING=${OPENMP_FLAGS_TEST}"
                "-DINCLUDE_DIRECTORIES:STRING=${OpenMP_${LANG}_INCLUDE_DIR}"
              LINK_LIBRARIES ${CMAKE_${LANG}_VERBOSE_FLAG}
                             ${OpenMP_libomp_LIBRARY})
          endif()
...
function(_OPENMP_GET_SPEC_DATE LANG SPEC_DATE)
  ...
  set(BIN_FILE
      "${CMAKE_BINARY_DIR}${CMAKE_FILES_DIRECTORY}/FindOpenMP/ompver_${LANG}.bin"
  )
  string(REGEX REPLACE "[-/=+]" "" OPENMP_PLAIN_FLAG "${OPENMP_FLAG}")
  try_compile(
    OpenMP_SPECTEST_${LANG}_${OPENMP_PLAIN_FLAG} SOURCE_FROM_VAR
    "${_OPENMP_TEST_SRC_NAME}" _OPENMP_TEST_SRC_CONTENT LOG_DESCRIPTION
    "Detecting ${LANG} OpenMP version"
...
foreach(LANG IN ITEMS C CXX)
  if(CMAKE_${LANG}_COMPILER_LOADED)
    if(NOT DEFINED OpenMP_${LANG}_FLAGS
       OR "${OpenMP_${LANG}_FLAGS}" STREQUAL "NOTFOUND"
      ...
      if(NOT DEFINED OpenMP_${LANG}_FLAGS OR "${OpenMP_${LANG}_FLAGS}" STREQUAL
                                             "NOTFOUND")
        set(OpenMP_${LANG}_FLAGS
            "${OpenMP_${LANG}_FLAGS_WORK}"
            CACHE STRING "${LANG} compiler flags for OpenMP parallelization"
                  FORCE)
      endif()
      if(NOT DEFINED OpenMP_${LANG}_LIB_NAMES OR "${OpenMP_${LANG}_LIB_NAMES}"
                                                 STREQUAL "NOTFOUND")
        set(OpenMP_${LANG}_LIB_NAMES
            "${OpenMP_${LANG}_LIB_NAMES_WORK}"
            CACHE STRING
                  "${LANG} compiler libraries for OpenMP parallelization" FORCE)
      endif()
...
if(CMAKE_Fortran_COMPILER_LOADED)
  if(NOT DEFINED OpenMP_Fortran_FLAGS
     OR "${OpenMP_Fortran_FLAGS}" STREQUAL "NOTFOUND"
     OR NOT DEFINED OpenMP_Fortran_LIB_NAMES
     OR "${OpenMP_Fortran_LIB_NAMES}" STREQUAL "NOTFOUND"
    ...
    if(NOT DEFINED OpenMP_Fortran_FLAGS OR "${OpenMP_Fortran_FLAGS}" STREQUAL
                                           "NOTFOUND")
    ...
    if(NOT DEFINED OpenMP_Fortran_LIB_NAMES OR "${OpenMP_Fortran_LIB_NAMES}"
                                               STREQUAL "NOTFOUND")
  ...
  if(CMAKE_${LANG}_COMPILER_LOADED)
    ...
    if("${OpenMP_${LANG}_LIB_NAMES}" STREQUAL "NOTFOUND")
      ...
      foreach(_OPENMP_IMPLICIT_LIB IN LISTS OpenMP_${LANG}_LIB_NAMES)
        list(APPEND _OPENMP_${LANG}_REQUIRED_LIB_VARS
             OpenMP_${_OPENMP_IMPLICIT_LIB}_LIBRARY)
      endforeach()
    endif()

    find_package_handle_standard_args(
      OpenMP_${LANG} NAME_MISMATCHED
    ...
    if(OpenMP_${LANG}_FOUND)
      if(DEFINED OpenMP_${LANG}_VERSION)
        if(NOT _OpenMP_MIN_VERSION OR _OpenMP_MIN_VERSION VERSION_GREATER
                                      OpenMP_${LANG}_VERSION)
      ...
      foreach(_OPENMP_IMPLICIT_LIB IN LISTS OpenMP_${LANG}_LIB_NAMES)
        list(APPEND OpenMP_${LANG}_LIBRARIES
             "${OpenMP_${_OPENMP_IMPLICIT_LIB}_LIBRARY}")
      endforeach()
      if(OpenMP_${LANG}_INCLUDE_DIR)
      ...
      if(OpenMP_${LANG}_FLAGS)
        separate_arguments(_OpenMP_${LANG}_OPTIONS NATIVE_COMMAND
                           "${OpenMP_${LANG}_FLAGS}")
        set_property(
          TARGET OpenMP::OpenMP_${LANG}
          PROPERTY INTERFACE_COMPILE_OPTIONS
                   "$<$<COMPILE_LANGUAGE:${LANG}>:${_OpenMP_${LANG}_OPTIONS}>")
        if(CMAKE_${LANG}_COMPILER_ID STREQUAL "Fujitsu"
           OR ${CMAKE_${LANG}_COMPILER_ID} STREQUAL "IntelLLVM")
          set_property(
            TARGET OpenMP::OpenMP_${LANG} PROPERTY INTERFACE_LINK_OPTIONS
                                                   "${OpenMP_${LANG}_FLAGS}")
        endif()
      ...
      if(OpenMP_${LANG}_INCLUDE_DIRS)
        set_property(
          ...
          PROPERTY INTERFACE_INCLUDE_DIRECTORIES
                   "$<BUILD_INTERFACE:${OpenMP_${LANG}_INCLUDE_DIRS}>")
      endif()
      if(OpenMP_${LANG}_LIBRARIES)
        set_property(
          TARGET OpenMP::OpenMP_${LANG} PROPERTY INTERFACE_LINK_LIBRARIES
                                                 "${OpenMP_${LANG}_LIBRARIES}")
      endif()
    endif()
  endif()
...
foreach(LANG IN ITEMS C CXX Fortran)
  if((NOT OpenMP_FIND_COMPONENTS AND CMAKE_${LANG}_COMPILER_LOADED)
