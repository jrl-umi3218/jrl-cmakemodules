...
MACRO(ADD_PROJECT_DEPENDENCY)
  ...
  # Add options OPTIONAL or REQUIRED
  cmake_parse_arguments(PARSED_ARGN
    "OPTIONAL;REQUIRED"
    "PKG_CONFIG_REQUIRES"
    ""
    ${ARGN})
  ...
  string(REPLACE ";" " " PACKAGE_ARGS
    "${PARSED_ARGN_UNPARSED_ARGUMENTS}")

  # If REQUIRED is not specified consider the package as optional
  # search for the package in a quiet mode.
  IF(PARSED_ARGN_OPTIONAL OR NOT PARSED_ARGN_REQUIRED)
    find_package(${PARSED_ARGN_UNPARSED_ARGUMENTS} QUIET)
  # Else generate an error
  ELSE(PARSED_ARGN_OPTIONAL OR NOT PARSED_ARGN_REQUIRED)
    find_package(${PARSED_ARGN_UNPARSED_ARGUMENTS} REQUIRED)
  ENDIF(PARSED_ARGN_OPTIONAL OR NOT PARSED_ARGN_REQUIRED)

  # If the package has been found provides the dependency
  IF(${${ARGV0}_FOUND})

    list(APPEND _PACKAGE_CONFIG_DEPENDENCIES_FIND_PACKAGE
      "  find_package(${PACKAGE_ARGS})")
    list(APPEND _PACKAGE_CONFIG_DEPENDENCIES_FIND_DEPENDENCY
      "  find_dependency(${PACKAGE_ARGS})")

    # Propagate variables changes to the cached values
    set(_PACKAGE_CONFIG_DEPENDENCIES_PROJECTS
      "${_PACKAGE_CONFIG_DEPENDENCIES_PROJECTS}" CACHE INTERNAL "")
    set(_PACKAGE_CONFIG_DEPENDENCIES_FIND_PACKAGE
      "${_PACKAGE_CONFIG_DEPENDENCIES_FIND_PACKAGE}" CACHE INTERNAL "")
    set(_PACKAGE_CONFIG_DEPENDENCIES_FIND_DEPENDENCY
      "${_PACKAGE_CONFIG_DEPENDENCIES_FIND_DEPENDENCY}" CACHE INTERNAL "")
  ENDIF(${${ARGV0}_FOUND})
