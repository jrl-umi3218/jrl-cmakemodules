...
MACRO(PKG_CONFIG_USE_LCOMPILE_DEPENDENCY TARGET PREFIX NO_INCLUDE_SYSTEM)
  ...
  IF(DEFINED ${PREFIX}_DEBUG_FOUND)
    FOREACH(FLAG ${${PREFIX}_DEBUG_CFLAGS_OTHER})
      TARGET_COMPILE_OPTIONS(${TARGET} ${PUBLIC_KEYWORD} "$<$<CONFIG:Debug>:${FLAG}>")
    ENDFOREACH()
    FOREACH(FLAG ${${PREFIX}_CFLAGS_OTHER})
      TARGET_COMPILE_OPTIONS(${TARGET} ${PUBLIC_KEYWORD} "$<$<NOT:$<CONFIG:Debug>>:${FLAG}>")
    ENDFOREACH()
  ELSE()
    FOREACH(FLAG ${${PREFIX}_CFLAGS_OTHER})
      TARGET_COMPILE_OPTIONS(${TARGET} ${PUBLIC_KEYWORD} ${FLAG})
    ENDFOREACH()
  ENDIF()
  ...
  IF(NOT ${NO_INCLUDE_SYSTEM})
    TARGET_INCLUDE_DIRECTORIES(${TARGET} SYSTEM ${PUBLIC_KEYWORD} ${${PREFIX}_INCLUDE_DIRS})
  ELSE()
    TARGET_INCLUDE_DIRECTORIES(${TARGET} ${PUBLIC_KEYWORD} ${${PREFIX}_INCLUDE_DIRS})
  ENDIF()
  IF(DEFINED ${PREFIX}_DEBUG_FOUND)
    IF(NOT ${NO_INCLUDE_SYSTEM})
      TARGET_INCLUDE_DIRECTORIES(${TARGET} SYSTEM ${PUBLIC_KEYWORD} ${${PREFIX}_DEBUG_INCLUDE_DIRS})
    ELSE()
      TARGET_INCLUDE_DIRECTORIES(${TARGET} ${PUBLIC_KEYWORD} ${${PREFIX}_DEBUG_INCLUDE_DIRS})
    ENDIF()
  ENDIF()
